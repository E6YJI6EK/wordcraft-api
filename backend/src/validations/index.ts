...
export const createDocumentSchema = z.object({
  ...
  type: z.enum(['coursework', 'thesis', 'report', 'essay'], {
    errorMap: () => ({ message: 'Тип документа должен быть одним из: coursework, thesis, report, essay' })
  }),
  content: z.string().optional().default(''),
  gostFormat: z.enum(['gost-7.32-2017', 'gost-7.1-2003', 'gost-2.105-95'], {
    errorMap: () => ({ message: 'Формат ГОСТ должен быть одним из: gost-7.32-2017, gost-7.1-2003, gost-2.105-95' })
  }).optional().default('gost-7.32-2017'),
  settings: z.object({
    fontSize: z.number()
  ...
  metadata: z.object({
    ...
    year: z.number()
      ...
      .max(new Date().getFullYear() + 1, 'Год не может быть больше текущего + 1')
      .optional(),
...
export const documentQuerySchema = z.object({
  page: z.string().transform(val => parseInt(val, 10)).pipe(
    z.number().min(1, 'Номер страницы должен быть не менее 1')
  ).optional().default('1'),
  limit: z.string().transform(val => parseInt(val, 10)).pipe(
    z.number().min(1, 'Лимит должен быть не менее 1').max(100, 'Лимит не должен превышать 100')
  ).optional().default('10'),
...
export const gostFormatSchema = z.object({
  ...
  format: z.enum(['gost-7.32-2017', 'gost-7.1-2003', 'gost-2.105-95'], {
    errorMap: () => ({ message: 'Формат ГОСТ должен быть одним из: gost-7.32-2017, gost-7.1-2003, gost-2.105-95' })
  }).optional(),
...
export const fileUploadSchema = z.object({
  file: z.object({
    ...
    mimetype: z.string().refine(
      (type) => ['application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/msword', 'text/plain'].includes(type),
